...
sub render_map {
    my $self = shift;
    my $individual_id = $self->get_map()->get_id();
    ...
    my $chr_q = "SELECT count(distinct(linkage_group.lg_name)) FROM phenome.phenome_genotype
    ...
    my $chr_h = $self->get_map()->get_dbh()->prepare($chr_q);
    ...
    my $sth = $self->get_map()->get_dbh()->prepare($query);
...
#    print STDERR "QUERY3: $query3\n";
    ...
    my $sth2 = $self->get_map()->get_dbh()->prepare($query2);
    $sth2->execute($individual_id);

    my $sth3 = $self->get_map()->get_dbh()->prepare($query3);
