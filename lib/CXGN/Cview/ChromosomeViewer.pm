...
CXGN::Cview::ChromosomeViewer -- a module to display comparative chromosome maps
...
ChromosomeViewer caches the images it produces using CXGN::Tools::WebImageCache. The expiration time is set to a day or so.
...
sub new {
    ...
    if ( !$dbh ) { die "ChromosomeViewer now takes a dbh parameter."; }
...
=head2 accessors set_state_hashref(), get_state_hashref()
  ...
  Property:	the state_hashref is a hash describing the current
                state of the ChromosomeViewer, essentially collecting
...
sub get_temp_dir {
    ...
    if ( !exists( $self->{temp_dir} ) ) {
        die "ChromosomeViewer: need a temp_dir  for storing images... STOP!\n";
    }
...
sub get_basedir {
    ...
    if ( !exists( $self->{basedir} ) ) {
        die "ChromosomeViewer: need a basedir for storing images... STOP!\n";
    }
    return $self->{basedir};
