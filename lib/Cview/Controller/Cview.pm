...
package Cview::Controller::Cview;
...
sub auto :Args(0) { 
    ...
    #
    $c->stash->{dbh} = CXGN::DB::Connection->new({
    ...
    $c->stash->{default_map_id} = $self->config->{cview_default_map_id};
    $c->stash->{referer} = $c->req->referer();
    $c->stash->{tempdir} = $c->config->{"tempfiles_subdir"}."/cview";
